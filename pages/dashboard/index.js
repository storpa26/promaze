...
import CommitteeDashboard from "../../components/DashboardComponent/CommitteeDashboard";
import FooterComponent from "../../components/FooterComponent/FooterComponent";

const Dashboard = () => {
    const [studentTeamId, setStudentTeamId] = useState();
    const [supervisorTeamId, setSupervisorTeamId] = useState();
        ...
        axios
            ...
            .then((response) => {
                if (response.data[0].role === "student") {
                    setStudentTeamId(response.data[0].teamStatus);
                    setUserRole(response.data[0].role);
                } else if (response.data[0].role === "teacher") {
                    setSupervisorTeamId(response.data[0].supervising);
                    setUserRole(response.data[0].role);
                } else {
                    setUserRole(response.data[0].role)
                }
